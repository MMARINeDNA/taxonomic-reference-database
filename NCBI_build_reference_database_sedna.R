...
reference_fasta_file_location <- "G:/My Drive/01 MMARINeDNA MURI/01 Module 3/02 Bioinformatic analysis/01.5 Reference Taxonomy"
setwd(reference_fasta_file_location)

# create NCBI search terms
organism <- c("cetacean")
locus <- list(c("dloop", "d-loop"),
	     c("control AND region"))
...
search.term <- paste0("(", organism[i], " AND (", paste(locus[[j]], collapse = " OR "), ")), NOT (", paste(exclude, collapse = " OR "), ")")
print(search.term)
...
write(fasta, file=paste(organism[i], "locus", j, "reference_fasta.fasta", sep = "_"))
...
fasta.file.list <- list.files(pattern = organism[i]) 
...
fasta.files[[i]] <- fasta.files[[i]][!(grepl("sp_AL", names(fasta.files[[i]])))]
}
...
#common.names <- do.call("bind_rows", common.names.list) %>% 
common.name <- bind_rows(common.names.list) %>%
...
reference.db.taxonomy.common <- reference.db.taxonomy %>% 
  left_join(common.name, by = c("species" = "species"))
